...
class RLRunner:
        ...
        while not stop_evt.is_set():
            ...
            if now - last_print > int(self.cfg.print_every_sec * 1000):
                if self.loss_ema_a is not None and self.loss_ema_c is not None:
                    ...
                    loss_str = ""
                print(f"[RL] epoch {self.epoch_idx}/{self.epoch_total} | steps env/train: {getattr(self.agent,'total_env_steps',0)}/{getattr(self.agent,'total_train_steps',0)} | replay={len(self.agent.buf)}{loss_str}")
                last_print = now
    ...
    def handle_requests(self) -> bool:
        ...
        for rq_path in rq_paths:
            ...
            # # === 记录 doc, a01, qp_f, qp ===
            # try:
            #     trace_path = os.path.join(self.cfg.rl_dir, f"qp_trace_e{self.epoch_idx:04d}.txt")
            #     qp_f = self.cfg.qp_min + float(a01) * (self.cfg.qp_max - self.cfg.qp_min)
            #     with open(trace_path, "a", encoding="utf-8") as fp:
            #         fp.write(f"{doc}\t{a01:.9f}\t{qp_f:.6f}\t{qp}\n")
            # except Exception as _e:
            #     print(f"[RL][WARN] write qp trace failed: {trace_path} ({_e})")
            #
            # # === 追加结束 ===
    ...
    def handle_feedbacks(self) -> bool:
        ...
        for fb_path in fb_paths:
            ...
            if self.cfg.mode == "train":
                mg_ctx = {"frames_so_far": int(max(1, st["frames"])), "ema_abs_q": float(self._rew_ema_q), "ema_abs_b": float(self._rew_ema_b)}
    ...
    def _write_epoch_metrics(self, epoch_id: int):
        try:
            ...
            with open(self._metrics_csv, "a", newline="") as f:
                ...
                w.writerow(row)
            print(f"[RL][CSV] epoch={epoch_id} mode={self.cfg.mode} avg_lossa={row['lossa_mean']} avg_lossc={row['lossc_mean']} | frames={self._ep_frames} avg_bpf={row['avg_bits_per_frame']} avg_psnr={row['avg_psnr_db']}" + (f" avg_kbps={row['avg_kbps']}" if row['avg_kbps'] != "" else ""))
        except Exception as e:
